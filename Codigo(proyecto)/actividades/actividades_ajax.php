<?php
	...
	if($action == 'ajax'){
		...
		$count_query   = mysqli_query($con,"SELECT count(*) AS numrows FROM actividades");
		if ($row= mysqli_fetch_array($count_query)){$numrows = $row['numrows'];}
		$total_pages = ceil($numrows/$per_page);
		$reload = 'ingreso_actividades.php';
		//consulta principal para recuperar los datos
		$query = mysqli_query($con,"SELECT * FROM actividades order by nombre LIMIT $offset,$per_page");
			...
			while($row = mysqli_fetch_array($query)){
				?>
					 <tr>
                          ...
                          <a  data-toggle="modal" data-target="#dataUpdate" data-id="<?php echo $row['id']?>" data-nombre="<?php echo $row['nombre']?>" 
                          data-fecha="<?php echo $row['fecha']?>" data-descripcion="<?php echo $row['descripcion']?>" data-tipo="<?php echo $row['tipo']?>" >
