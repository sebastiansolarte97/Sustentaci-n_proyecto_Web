...
1. select SELECT e.id_estado,m.nombre_mate,e.nota, e.id_estudiante, p.nombre_est, p.apellido_est,c.nombre_curso  FROM estado_academico e
      inner join estudiante p on e.id_estudiante=p.id_estudiante inner join materia m on e.id_materia=m.id_materia inner join cursos c on e.id_curso=c.id_cursos
...
6.	
SELECT e.id, e.falta, e.fecha, e.id_estudiante, p.nombre_est, p.apellido_est,c.nombre_curso  FROM llamados_atencion e
      inner join estudiante p on e.id_estudiante=p.id_estudiante  inner join cursos c on e.id_curso=c.id_cursos
